...
class IncomeStatement extends FinancialStatement
{
    ...
    {
        return array_merge(
            config('accounting')[self::OPERATING_REVENUES],
            config('accounting')[self::NON_OPERATING_REVENUES],
            config('accounting')[self::OPERATING_EXPENSES],
            config('accounting')[self::NON_OPERATING_EXPENSES]
        );
    }
    ...
    public static function getResults($month, $year, Entity $entity = null)
    {
        ...
        $revenues = self::getBalance(config('accounting')[self::OPERATING_REVENUES], $startDate, $endDate, $entity);

        $otherRevenues = self::getBalance(config('accounting')[self::NON_OPERATING_REVENUES], $startDate, $endDate, $entity);

        $cogs = self::getBalance(config('accounting')[self::OPERATING_EXPENSES], $startDate, $endDate, $entity);

        $expenses = self::getBalance(config('accounting')[self::NON_OPERATING_EXPENSES], $startDate, $endDate, $entity);
    ...
    private static function getBalance(array $accountTypes, Carbon $startDate, Carbon $endDate, Entity $entity = null): float
    {
        ...
        $accountTable = config('accounting.table_prefix') . 'accounts';
        $ledgerTable = config('accounting.table_prefix') . 'ledgers';
