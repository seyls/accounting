...
class BalanceSheet extends FinancialStatement
{
    ...
    {
        return array_merge(
            config('accounting')[BalanceSheet::ASSETS],
            config('accounting')[BalanceSheet::LIABILITIES],
            config('accounting')[BalanceSheet::EQUITY],
            config('accounting')[BalanceSheet::RECONCILIATION]
        );
    }
    ...
    public function getSections($startDate = null, $endDate = null, $fullbalance = true): array
    {
        ...
        $this->accounts[self::EQUITY][self::NET_PROFIT][config('accounting')['statements'][self::NET_PROFIT]] = [
            "accounts" => null,
            "total" => $netProfit,
            "id" => 0
        ];
