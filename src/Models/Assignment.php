...
use Seyls\Accounting\Reports\AccountSchedule;
...
use Seyls\Accounting\Exceptions\OverClearance;
use Seyls\Accounting\Exceptions\SelfClearance;
use Seyls\Accounting\Exceptions\NegativeAmount;
use Seyls\Accounting\Exceptions\MixedAssignment;
use Seyls\Accounting\Exceptions\UnpostedAssignment;
use Seyls\Accounting\Exceptions\InsufficientBalance;
use Seyls\Accounting\Exceptions\MissingForexAccount;
use Seyls\Accounting\Exceptions\InvalidClearanceEntry;
use Seyls\Accounting\Exceptions\UnclearableTransaction;
use Seyls\Accounting\Exceptions\InvalidClearanceAccount;
use Seyls\Accounting\Exceptions\UnassignableTransaction;
use Seyls\Accounting\Exceptions\InvalidClearanceCurrency;
use Seyls\Accounting\Exceptions\InvalidTransaction;
...
class Assignment extends Model implements Segregatable
{
    ...
    public function save(array $options = []): bool
    {
        ...
        if (!bccomp($transactionRate, $clearedRate, config('accounting.forex_scale')) == 0) {
    ...
    private function validate(float $transactionRate, float $clearedRate, string $transactionType, string $clearedType): void
    {
        ...
        if (round($this->transaction->balance, config('accounting.forex_scale')) < round($this->amount, config('accounting.forex_scale'))) {
        ...
        if (!bccomp($transactionRate, $clearedRate, config('accounting.forex_scale')) == 0 && !isset($this->forex_account_id)) {
