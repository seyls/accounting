...
class Account extends Model implements Recyclable, Segregatable
{
    ...
    {
        return config('accounting')['accounts'][$type];
    }
        ...
        foreach (Account::whereIn('account_type', $accountTypes)->where('entity_id', '=', $entity->id)->get() as $account) {
            ...
            if ($account->closingBalance <> 0 || $account->balanceMovement <> 0) {
                ...
                if (is_null($account->category)) {
                    $categoryName = config('accounting')['accounts'][$account->account_type];
                    $categoryId = 0;
                } else {
    ...
    {
        $transactionsTable = config('accounting.table_prefix') . 'transactions';
        $closingTransactionsTable = config('accounting.table_prefix') . 'closing_transactions';
    ...
    private function getAccountCode(): int
    {
        ...
        return config('accounting')['account_codes'][$this->account_type] + $query->count() + 1;
    }
