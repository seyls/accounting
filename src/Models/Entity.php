...
use Seyls\Accounting\Exceptions\MissingReportingCurrency;
use Seyls\Accounting\Exceptions\UnconfiguredLocale;
...
class Entity extends Model implements Recyclable
{
    ...
    {
        return $this->hasMany(config('accounting.user_model'));
    }
    ...
    public function save(array $options = []): bool
    {
        if (is_null($this->locale)) {
            $this->locale = config('accounting.locales')[0];
        } else {
            if (!in_array($this->locale, config('accounting.locales'))) {
