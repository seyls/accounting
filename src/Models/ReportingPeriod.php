...
use Seyls\Accounting\Reports\BalanceSheet;
...
class ReportingPeriod extends Model implements Segregatable, Recyclable
{
            ...
            if (!$account->isClosed($this->calendar_year)) {
                ...
                if (array_sum($balances) <> 0) {
                    foreach ($this->transactionCurrencies($account->id) as $currency) {
                        ...
                        if ($localBalance <> round($foreignBalance, config('accounting.forex_scale'))) {
    ...
    {
        $reportingCurrency = $this->entity->currency_id;
        $transactionTable = config('accounting.table_prefix') . 'transactions';
        $currenciesTable = config('accounting.table_prefix') . 'currencies';
    ...
    ): Transaction
    {
        ...
        $isAsset = in_array($account->account_type, config('accounting')[BalanceSheet::ASSETS]);
        $isLiability = in_array($account->account_type, config('accounting')[BalanceSheet::LIABILITIES]);
