...
use Seyls\Accounting\Exceptions\MissingLineItem;
use Seyls\Accounting\Exceptions\HangingClearances;
use Seyls\Accounting\Exceptions\PostedTransaction;
use Seyls\Accounting\Exceptions\UnpostedAssignment;
use Seyls\Accounting\Exceptions\RedundantTransaction;
use Seyls\Accounting\Exceptions\ClosedReportingPeriod;
use Seyls\Accounting\Exceptions\InvalidTransactionDate;
use Seyls\Accounting\Exceptions\AdjustingReportingPeriod;
use Seyls\Accounting\Exceptions\InvalidCurrency;
use Seyls\Accounting\Exceptions\InvalidTransactionType;
use Seyls\Accounting\Exceptions\UnbalancedTransaction;
...
class Transaction extends Model implements Segregatable, Recyclable, Clearable, Assignable
{
    ...
    {
        $this->table = config('accounting.table_prefix') . 'transactions';
    ...
    {
        return config('accounting')['transactions'][$type];
    }
    ...
    {
        if (in_array($lineItem->account->account_type, config('accounting.single_currency')) && $lineItem->account->currency_id != $this->currency_id) {
    ...
    public function save(array $options = []): bool
    {
        ...
        if (in_array($this->account->account_type, config('accounting.single_currency')) &&
    ...
    public function getHasIntegrityAttribute(): bool
    {
        ...
        return $this->ledgers->every(
            function ($ledger, $key) {
                return hash(config('accounting')['hashing_algorithm'],$ledger->hashed()) == $ledger->hash;
            }
        );
    }
}
