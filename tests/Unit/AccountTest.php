...
use Seyls\Accounting\Exceptions\HangingTransactions;
use Seyls\Accounting\Exceptions\InvalidCategoryType;
use Seyls\Accounting\Exceptions\MissingAccountType;
...
class AccountTest extends TestCase
{
    ...
    {
        $type = $this->faker->randomElement(array_keys(config('accounting')['accounts']));
    ...
    public function testAccountCodes()
    {
        ...
        $this->assertEquals(config('accounting')['account_codes'][Account::NON_CURRENT_ASSET] + 1, $account->code);
        ...
        $this->assertEquals(config('accounting')['account_codes'][Account::OPERATING_REVENUE] + 4, $account->code);
        ...
        $this->assertEquals(config('accounting')['account_codes'][Account::CURRENT_LIABILITY] + 13, $account->code);
        ...
        $this->assertEquals(config('accounting')['account_codes'][Account::BANK] + 1, $account->code);

        $account->account_type = Account::RECEIVABLE;
        $account->save();

        $this->assertEquals(config('accounting')['account_codes'][Account::RECEIVABLE] + 2, $account->code);
        ...
        $this->assertEquals(config('accounting')['account_codes'][Account::OPERATING_REVENUE] + 1, $account->code);
    }
